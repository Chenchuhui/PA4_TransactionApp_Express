...
router.get('/todo/',
    ...
    async(req, res, next) => {
        ...
        res.locals.items =
            await ToDoItem.find({ userId: req.user._id, completed: completed }).sort({ completed: 1, priority: 1, createdAt: 1 })
        res.render('toDoList');
    });
...
router.get('/todo/complete/:itemId',
    ...
    async(req, res, next) => {
        ...
        await ToDoItem.findOneAndUpdate({ _id: req.params.itemId }, { $set: { completed: true } });
        res.redirect('/toDo')
    });
...
router.get('/todo/uncomplete/:itemId',
    ...
    async(req, res, next) => {
        console.log("inside /todo/complete/:itemId")
        await ToDoItem.findOneAndUpdate({ _id: req.params.itemId }, { $set: { completed: false } });
        res.redirect('/toDo')
    });
